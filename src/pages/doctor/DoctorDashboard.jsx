...
export default function DoctorDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-16 pb-safe select-none">
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur-md px-4 sm:px-6 py-3 sm:py-4 pt-safe shadow-xs">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-blue-50/70 border border-blue-100 text-xs">
              <div className="size-2 rounded-full bg-[#174ea6] animate-ping" />
              <span className="font-bold text-[#174ea6]">{DEMO_DOCTOR.name}</span>
              <span className="text-[10px] text-slate-500">({DEMO_DOCTOR.specialty})</span>
            </div>
            <button
              onClick={() => navigate('/')}
        ...
        <motion.div initial="hidden" animate="visible" className="space-y-6">
          ...
          <motion.div custom={0} variants={fadeIn} className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium mt-1 flex items-center gap-1.5">
                <Users className="size-4 text-[#174ea6]" />
          ...
          <motion.div custom={0.5} variants={fadeIn} className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-xs">
            <div className="flex flex-col items-center justify-center text-center p-2.5 rounded-xl bg-slate-50">
              <span className="font-mono text-xl sm:text-2xl text-[#174ea6] font-bold leading-tight">14</span>
            ...
            <div className="flex flex-col items-center justify-center text-center p-2.5 rounded-xl bg-slate-50">
              <span className="font-mono text-xl sm:text-2xl text-[#174ea6] font-bold leading-tight">9m</span>
          ...
          <motion.div custom={1} variants={fadeIn} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <button
              ...
              className={`p-5 text-left rounded-2xl border transition cursor-pointer flex items-center gap-4 ${
                filterPriority === 'priority'
                  ? 'border-red-500 bg-white ring-2 ring-red-500/20 shadow-xs'
                  : 'border-slate-200/80 bg-white hover:bg-slate-50'
              }`}
            >
              <div className="size-12 rounded-2xl bg-red-50 flex items-center justify-center shrink-0 text-red-600 border border-red-200">
            ...
            <button
              ...
              className={`p-5 text-left rounded-2xl border transition cursor-pointer flex items-center gap-4 ${
                filterPriority === 'ready'
                  ? 'border-[#174ea6] bg-white ring-2 ring-blue-500/20 shadow-xs'
                  : 'border-slate-200/80 bg-white hover:bg-slate-50'
              }`}
            >
              <div className="size-12 rounded-2xl bg-blue-50 flex items-center justify-center shrink-0 text-[#174ea6] border border-blue-200">
            ...
            <button
              ...
              className={`p-5 text-left rounded-2xl border transition cursor-pointer flex items-center gap-4 ${
                filterPriority === 'review'
                  ? 'border-amber-400 bg-white ring-2 ring-amber-400/20 shadow-xs'
                  : 'border-slate-200/80 bg-white hover:bg-slate-50'
              }`}
            >
          ...
          <motion.div custom={3} variants={fadeIn} className="space-y-3">
            ...
            <div className="flex items-center justify-between px-1">
              ...
              <span className="text-xs text-[#174ea6] font-semibold flex items-center gap-1">
                <span className="size-2 rounded-full bg-[#174ea6] animate-pulse" />
                Demo ABHA record
              </span>
            </div>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className={`size-11 rounded-2xl flex items-center justify-center font-bold text-xs shrink-0 ${
                            hasCritical ? 'bg-red-500 text-white shadow-xs' : 'bg-blue-50 text-[#174ea6] border border-blue-200'
                          }`}>
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 text-[11px] text-slate-500 mt-0.5">
                              <span className="flex items-center gap-1 font-mono text-[#174ea6] font-semibold">
                                <Clock className="size-3" /> Waiting ~{i * 4 + 6}m
                              </span>
                              <span>•</span>
                              <span className="text-[#174ea6] font-bold">ABHA Verified</span>
                            </div>
                          </div>
                        </div>

                        {/* AI Readiness Pill */}
                        <div className="flex flex-col items-center shrink-0 bg-blue-50/60 px-2 py-1 rounded-xl border border-blue-100">
                          <span className="font-mono text-xs font-extrabold text-[#174ea6]">
                            {hasCritical ? '92%' : '88%'}
                          </span>
            ...
            {/* DESKTOP TABLE VIEW */}
            <div className="hidden md:block bg-white border border-slate-200 shadow-xs rounded-2xl overflow-hidden">
                    ...
                    filteredPatients.map((patient, i) => {
                      ...
                      return (
                        <motion.div
                          ...
                          variants={fadeIn}
                          onClick={() => navigate(`/doctor/patient/${patient.id}`)}
                          className={`grid grid-cols-12 gap-3 px-6 py-4 items-center border-b border-slate-100 last:border-b-0 hover:bg-blue-50/40 cursor-pointer transition-colors ${
                        ...
                        >
                          ...
                          <div className="col-span-2 flex items-center justify-end gap-2">
                              ...
                              {patient.clinicalSignals.slice(0, 3).map((sig, j) => (
                                <span
                                  ...
                                  className={`size-2.5 rounded-full inline-block ${
                                    sig.severity === 'critical' ? 'bg-red-500 animate-ping' :
                                    sig.severity === 'high' ? 'bg-amber-500' :
                                    sig.severity === 'medium' ? 'bg-amber-300' : 'bg-[#174ea6]'
                                  }`}
                            ...
                            <button
                              type="button"
                              className="size-8 rounded-full border border-slate-200 flex items-center justify-center text-slate-400 hover:text-[#174ea6] hover:border-[#174ea6] transition cursor-pointer"
                            >
